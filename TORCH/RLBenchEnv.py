...
#         self.logger.propagate = 0
    ...
    def _get_state(self, obs): #on ne retourne que les bon elements
        if len(self.state_type_list) > 0:
            if self._observation_mode == 'state':
                obs = self.get_low_dim_data(obs)

                target_pose = np.expand_dims(obs[-3:], axis=0)
                joints_pose = np.expand_dims(obs[8:15], axis=0)
                return np.concatenate((joints_pose, target_pose), axis=1)
